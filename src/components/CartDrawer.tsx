...
export const CartDrawer = ({
  ...
  const handleOrder = async () => {
    ...
    // YENİ: Masa ve Telefon numarası doğrulama
    if (!tableNumber.trim()) {
    ...
    try {
      const orderData = {
        ...
        phone: phoneNumber.trim(), // YENİ: Telefon numarası payload'a eklendi
      ...
      clearCart();
      setTableNumber("");
      setPhoneNumber(""); // Başarılı sipariş sonrası temizle
      onClose();
    } catch (error) {
            ...
            {cart.length === 0 ? (
              <div className="text-center text-gray-500 mt-20 flex flex-col items-center">
                ...
                <p className="text-sm mt-2">
                  Lezzetli menümüzden ürünler eklemeye başlayın!
                </p>
              </div>
            ) : (
              <ul className="space-y-6">
                {cart.map((item) => {
                  ...
                  const itemTotal =
                    (item.product.price + extrasTotal) * item.quantity;

                  return (
                    <li
                      key={item.id}
                      className="flex py-2 bg-white rounded-xl border border-gray-100 p-3 shadow-sm">
                      <img
                      ...
                      <div className="ml-4 flex-1 flex flex-col">
                        <div>
                          <div className="flex justify-between text-base font-bold text-brand-dark">
                            ...
                            <p className="ml-4 text-brand-orange">
                              {itemTotal.toFixed(2)} ₺
                            </p>
                          </div>
                        ...
                        <div className="flex-1 flex items-end justify-between text-sm mt-4">
                          <div className="flex items-center border border-gray-200 rounded-lg bg-gray-50">
                            ...
                            <span className="px-3 font-bold text-brand-dark">
                              {item.quantity}
                            </span>
                            <button
                              onClick={() =>
                          ...
                          <button
                            ...
                            className="font-medium text-red-500 hover:text-red-700 flex items-center gap-1 bg-red-50 px-2 py-1 rounded-md transition-colors">
                            <Trash2 size={14} /> Sil
                          </button>
                        </div>
                      </div>
            ...
            <div className="border-t border-gray-200 p-6 bg-white shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
              ...
              <div className="mb-3">
                <label className="block text-sm font-bold text-gray-700 mb-1.5">
                  Masa Numaranız <span className="text-red-500">*</span>
                </label>
                <input
                  type="text"
                  value={tableNumber}
                  ...
                  placeholder="Örn: 12"
                  className={`w-full border ${error && !tableNumber ? "border-red-500 ring-1 ring-red-500" : "border-gray-300"} rounded-lg px-4 py-3 focus:ring-2 focus:ring-brand-orange focus:border-brand-orange outline-none transition-all`}
                />
              </div>

              {/* TELEFON NUMARASI ALANI (YENİ) */}
              <div className="mb-5">
                <label className="block text-sm font-bold text-gray-700 mb-1.5">
                ...
                <input
                  ...
                  className={`w-full border ${error && (!phoneNumber || phoneNumber.length < 10) ? "border-red-500 ring-1 ring-red-500" : "border-gray-300"} rounded-lg px-4 py-3 focus:ring-2 focus:ring-brand-orange focus:border-brand-orange outline-none transition-all`}
                />
                {error && (
